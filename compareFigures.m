function differences = compareFigures(info1, info2, debug)
   ...
   if length(info1) ~= length(info2)
       ...
       differences(1).baseline = string([info1.class]);
       differences(1).test = string([info2.class]); 
       differences(1).matches = false;
       return;
   end
   
   % Check all classes
   classes1 = string([info1.class]);
   classes2 = string([info2.class]);
   if ~isequal(classes1, classes2)
       differences(1).class = 'root';
       differences(1).parent = '';
       differences(1).field = 'class_mismatch';
       differences(1).baseline = classes1;
       differences(1).test = classes2;
       differences(1).matches = false;
       return;
   end
